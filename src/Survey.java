...
public class Survey extends HttpServlet {
	...
	private static final int NEED_PROJECT_CHAR = 1;
	private static final int NEED_PROJECT_DATA = 2;
	private static final int DISPLAY_ALL_DATA = 3;
	 ...
	 * characters) and then if so if the name is possible to add to the database. It
	 * is not possible to add an already existing name to the database.
	 */
		...
		case NEED_NAME: // First state: get user name
			...
			if (name != null) {
				if (nameOk(name)) {
					...
					state = NEED_PROJECT_CHAR;
					out.println(formGenerator.projectCharRequestForm());
				} else {
			...
			break;
		case NEED_PROJECT_CHAR:
			int s0 = 0, s1 = 0;
			name = (String) session.getAttribute("name");
			String s0String = request.getParameter("s0");
			String s1String = request.getParameter("s1");
			if (s0String == null)
				out.println(formGenerator.projectCharRequestForm()); // first time
			else {
				boolean valuesOk = true;
				try {
					s0 = Integer.parseInt(s0String);
					s1 = Integer.parseInt(s1String);
				} catch (NumberFormatException e) {
					valuesOk = false;
				}
				valuesOk = valuesOk && valueOk(s0) && valueOk(s1);

				// display the next page
				if (valuesOk) {
					double percentage = (1.0 * s1) / (1.0 * s0);
					...
					out.println("<p> " + percentage + "are bosses");
					db.addProjectChar(name, s0, s1);
		...
		case NEED_PROJECT_DATA:
			...
			if (s11String == null)
				...
				if (valuesOk) {
					...
					db.addAnswers(name, s11, s12, s13, s14);
